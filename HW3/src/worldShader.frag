...
void main()
{
    ...
    vec3 spec = pow(max(dot(reflect(-normalize(LightVector - 0.5), Normal), normalize(CameraVector + 5)), 0.0f), SpecularExponent) * (specularReflectenceCoefficient * specularLightColor).xyz;
