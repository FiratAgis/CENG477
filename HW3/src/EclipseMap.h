...
public:
    ...
    unsigned int worldText, worldNormal;
    float imageHeight;
    float imageWidth;
    float radius = 600;
    ...
    unsigned int moonText, moonNormal;
    ...
    glm::mat4 view, normal, projection, mvpMoon, mvpWorld;
    glm::vec3 center;
    ...
    glm::mat4 modelMoon = glm::translate(glm::mat4(), glm::vec3(0.0f, 2600.0f, 0.0f));

    float cameraStartSpeed = 0.0f;
    float cameraSpeed = cameraStartSpeed;
    ...
    void setOthers(GLuint ShaderID, bool is_moon);

    void cameraSetGL(GLuint ShaderID);

    void bindBuffer(unsigned int vbo, unsigned int ebo, unsigned int normal, unsigned int text);
