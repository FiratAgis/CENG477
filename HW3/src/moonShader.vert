...
void main()
{
    ...
    LightVector = normalize(lightPosition - (MVP * vec4(VertexPosition, 1.0f)).xyz);
    CameraVector = normalize(cameraPosition - (MVP * vec4(VertexPosition, 1.0f)).xyz);

    Normal = normalize((MVP * vec4(VertexNormal, 1.0f)).xyz - (MVP * vec4(VertexPosition, 1.0f)).xyz);
    TexCoord = VertexTex;
